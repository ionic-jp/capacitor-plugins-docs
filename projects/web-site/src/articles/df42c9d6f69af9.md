...
"test": {
  ...
  "options": {
    ...
    "runner": "vitest",
    "runnerConfig": "vitest.config.ts", // jsdom設定が要る場合だけ（後述）
    "coverage": false
  },
...
{
  "compilerOptions": {
    "types": ["vitest/globals"]  // ["jasmine"] から変更
  },
  "include": ["src/**/*.spec.ts"]
}
```
...
vi.spyOn(obj, 'method'); // callThrough相当がデフォルト動作
```
...
```bash
grep -rl "@ionic/angular" node_modules/<パッケージ名>
```
