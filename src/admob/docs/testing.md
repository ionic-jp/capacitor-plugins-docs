...
`resetConsentInfo()` is for tests only. See [Consent](/docs/consent).

## Server-side verification

Server-side verification (SSV) callbacks fire only for production ads. Test ads will not hit your SSV endpoint. For a mock request example, see [Full-screen Ads](/docs/full-screen-ads).
