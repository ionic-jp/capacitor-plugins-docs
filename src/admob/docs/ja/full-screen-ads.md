...
表示前に広告を準備します。[初期化](/docs/configuration) と [同意](/docs/consent) のあとで呼び出します。先にリスナーを登録し、開発中はテスト広告ユニットを使い、体験の区切りでのみ表示します。共通のロード・表示・売上イベントは [広告イベント](/docs/events) にあります。
