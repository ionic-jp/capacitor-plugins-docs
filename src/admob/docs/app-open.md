...
Call this after [initialize](/docs/configuration) and [consent](/docs/consent). Load the ad ahead of time and confirm it is available before presentation. Do not block app startup indefinitely while waiting for an ad to load.
...
Use the `Closed` event to resume your app flow and begin loading the next ad. When no `adId` is passed to `showAppOpen()` or `isAppOpenLoaded()`, the most recently loaded ad is targeted. See [Ad Events](/docs/events).
