...
**iOS 15 or later is required. Brother does not distribute its SDK through CocoaPods or Swift
Package Manager, so download the SDK and expose the XCFramework as the local package described
below. The plugin supports Capacitor projects that use either package manager.**
...
The `BRLMPrinterKit.xcframework` file is the Brother Print SDK library, which you can download from the Brother website: https://support.brother.co.jp/j/s/es/dev/ja/mobilesdk/ios/index.html?c=jp&lang=ja&navi=offall&comple=on&redirect=on#ver4
...
2. If your Capacitor project uses CocoaPods, update its `ios/App/Podfile`.
...
Then run `pod update` in the `ios` directory. If your project uses Swift Package Manager, the local
`Package.swift` above is used instead; run `npx cap sync ios` after placing the package.
