...
export class AppDocs {
  ...
  async componentWillLoad(): Promise<void> {
    const { markdownContent, codes, activeLine, editFile } = await this.initialize(this.path);
  ...
  render() {
    return (
      <div class="app-docs">
        <main>
          <app-parser markdownContent={this.markdownContent}
                      hideCodeBlock={Object.keys(this.codes).length > 0}></app-parser>
          {Object.keys(this.codes).length > 0 ?
            <app-codes codes={this.codes} activeLine={this.activeLine}></app-codes> : ''}
        </main>
        <footer>
          <a class="edit-page" href={this.editFile} target="_blank">
