...
Tap to Pay collects contactless payments on a compatible phone or tablet without a separate card reader. Use `TerminalConnectTypes.TapToPay` after [configuration](/docs/configuration) and a working [connection token](/docs/collect-a-payment).
...
Android `initialize` requests the location permission listed in [Configuration](/docs/configuration). Bluetooth permissions are requested only when discovering `Bluetooth` or `Simulated` readers; Tap to Pay discovery itself does not request them.
...
7. Collect and confirm a `card_present` PaymentIntent as in [Collect a Payment](/docs/collect-a-payment).
...
- Optional reader software updates still follow [Reader Lifecycle](/docs/reader-lifecycle) rules: do not install during checkout.
